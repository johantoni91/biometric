...
@section("content")
	<div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
		<form action="{{ route("users.update", auth()->user()->id) }}"
			method="POST"
			...
			<div class="my-5 flex flex-col gap-2 md:grid md:grid-cols-12">
				...
				<div class="col-span-3">
					...
					<input
						...
						name="satker"
						type="text"
						value="{{ auth()->user()->satker ?? old("satker") }}"
						placeholder="Satker"
						required />
					@error("satker")
						<small class="text-red-500">{{ $message }}</small>
				...
				<div class="col-span-3">
					...
					<select
						...
						<option value="superadmin"
							{{ (auth()->user()->role || old("role")) == "superadmin" ? "selected" : "" }}>Superadmin</option>
						<option value="admin"
							{{ (auth()->user()->role || old("role")) == "admin" ? "selected" : "" }}>Admin</option>
						<option value="pegawai"
							{{ (auth()->user()->role || old("role")) == "pegawai" ? "selected" : "" }}>Pegawai</option>
					</select>
					@error("role")
						<small class="text-red-500">{{ $message }}</small>
					@enderror
